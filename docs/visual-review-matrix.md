...
The Monospace demo homepage was then migrated from its maintainer-oriented
Theme Lab page to the shared Flowershow showcase. The new page leads with
`A sharper home for your Markdown`, introduces Flowershow, demonstrates publishing
use cases and benefits, links the real kitchen sink and blog, and retains a
small theme-specific Preview description. It uses the real Flowershow navbar;
there is no landing-owned `<header>` or `<nav>`.

The focused audit covered the homepage, kitchen sink, blog list, and first blog
post at 1280 × 900 and 390 × 844 in light and dark: 16 renders. Headless Chrome
confirmed HTTP 200, the requested active mode, viewport-width containment,
working homepage calls to action, one homepage `h1`, a visible Preview marker,
and absence of the removed Theme Lab/release-audit copy. Full-page captures of
all homepage combinations and representative internal pages were manually
inspected. The aligned real navbar, editorial hero, use-case cards, dark
benefit band, publishing steps, theme sample, and final call to action remain
clear at both viewports; internal pages retain their intended theme treatment.

The first run exposed a 2652 px overflow. Root-cause inspection showed that
blank lines inside the indented template ended its Markdown HTML block, causing
later sections to render as wide literal code. The template now remains one
single raw-HTML block, protected by a regression test. Six cache-busted probes
and the repeated 16-render audit confirmed the correction. Flowershow's hosted
“Built with” badge remains a platform overlay and appears on internal pages as
well; it is not generated by the showcase.
